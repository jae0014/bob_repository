...
public class RecipeDetailServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 레시피 완성사진 = 썸네일
		 ...
		 for(int i = 0; i<imgList.size(); i++) { 
			 if(imgList.get(i).getfLevel()== 1) {
				 ...
				 * else if(imgList.get(i).getfLevel()==4) { recipeStep = imgList.get(i); }
				 */
		 }
		 //썸네일 thumbnail = imgList.get(i); }else if(imgList.get(i).getfLevel()==4)
		 //레시피 스텝 recipeStep = imgList.get(i); } }
		...
		if(rlist !=null) {
			...
			/* request.setAttribute("rSteList", rStepList); */
